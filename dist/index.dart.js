  ...
  var dart = [["_foreign_helper", "dart:_foreign_helper",, H, {
    ...
    ListMapView: {
      ...
      containsKey$1: function(key) {
        return typeof key === "number" && Math.floor(key) === key && key >= 0 && key < J.get$length$asx(this._values);
      },
    ...
    Primitives_valueFromDecomposedDate: function(years, month, day, hours, minutes, seconds, milliseconds, isUtc) {
      ...
      H.checkInt(minutes);
      H.checkInt(seconds);
      value = Date.UTC(years, month - 1, day, hours, minutes, seconds, milliseconds);
    ...
    ListMixin: {
      ...
      contains$1: function(receiver, element) {
        var t1, t2, i;
        for (t1 = receiver.length, t2 = this.get$length(receiver) !== t1, i = 0; i < t1; ++i)
          if (t2)
            throw H.wrapException(new P.ConcurrentModificationError(receiver));
        return false;
      },
    ...
    main: [function() {
      ...
      E.sendRequest((t1 && C.Location_methods).get$origin(t1)).then$1(new E.main_closure());
    }, "call$0", "index__main$closure", 0, 0, 1],
    headerStrParser: function(headerStr) {
      var headerMap = new H.JsLinkedHashMap(0, null, null, null, null, null, 0, [null, null]);
      C.JSArray_methods.forEach$1(headerStr.split("\n"), new E.headerStrParser_closure(headerMap));
      return headerMap;
    },
    sendRequest: function(url) {
      ...
      C.HttpRequest_methods._addEventListener$3(request, "readystatechange", new E.sendRequest_closure(new P._AsyncCompleter(t1, [null]), request), null);
      request.send();
      return t1;
    },
    main_closure: {
      "^": "Closure:10;",
      call$1: function(headers) {
        var serverDateStr, localDate, t1, map, t2, days, m, t3, yyyy, hhmmss, hhmmssList, hh, mm, ss, serverDate, diffHours;
        serverDateStr = J.$index$asx(headers, "Date");
        localDate = new P.DateTime(Date.now(), false);
        t1 = J.getInterceptor$asx(serverDateStr);
        if (t1.contains$1(serverDateStr, "GTM") === true)
          H.throwExpression(P.ArgumentError$("$GTMStr not a GMT time string"));
        t1 = C.JSString_methods.split$1(t1.replaceAll$2(serverDateStr, ",", ""), P.RegExp_RegExp("\\s+", true, false));
        map = new H.ListMapView(t1, [H.getTypeArgumentByIndex(t1, 0)]);
        if (map.containsKey$1(0))
          if (0 < 0 || 0 >= t1.length)
            return H.ioore(t1, 0);
        if (map.containsKey$1(1)) {
          if (1 < 0 || 1 >= t1.length)
            return H.ioore(t1, 1);
          t2 = t1[1];
        } else
          t2 = null;
        days = H.Primitives_parseInt(t2, null, null);
        if (map.containsKey$1(2)) {
          if (2 < 0 || 2 >= t1.length)
            return H.ioore(t1, 2);
          t2 = t1[2];
        } else
        ...
        if (!C.JSArray_methods.contains$1(m, t2))
          H.throwExpression(P.ArgumentError$value(t2, "not a month short name", null));
        t2 = C.JSArray_methods.indexOf$1(m, t2);
        if (map.containsKey$1(3)) {
          if (3 < 0 || 3 >= t1.length)
            return H.ioore(t1, 3);
          t3 = t1[3];
        } else
          t3 = null;
        yyyy = H.Primitives_parseInt(t3, null, null);
        if (map.containsKey$1(4)) {
          if (4 < 0 || 4 >= t1.length)
            return H.ioore(t1, 4);
          hhmmss = t1[4];
        } else
          hhmmss = null;
        if (map.containsKey$1(5))
          if (5 < 0 || 5 >= t1.length)
            return H.ioore(t1, 5);
        hhmmssList = J.split$1$s(hhmmss, ":");
        if (0 >= hhmmssList.length)
          return H.ioore(hhmmssList, 0);
        hh = H.Primitives_parseInt(hhmmssList[0], null, null);
        if (1 >= hhmmssList.length)
          return H.ioore(hhmmssList, 1);
        mm = H.Primitives_parseInt(hhmmssList[1], null, null);
        if (2 >= hhmmssList.length)
          return H.ioore(hhmmssList, 2);
        ss = H.Primitives_parseInt(hhmmssList[2], null, null);
        serverDate = new P.DateTime(H.checkInt(H.Primitives_valueFromDecomposedDate(yyyy, t2 + 1, days, hh, mm, ss, 0, true)), true).toLocal$0();
        diffHours = H.Primitives_getHours(serverDate) - H.Primitives_getHours(localDate);
        H.Primitives_getSeconds(serverDate);
        H.Primitives_getSeconds(localDate);
        if (diffHours >= 2) {
          window;
          t1 = "Server time over computer time " + diffHours;
          if (typeof console != "undefined")
            console.error(t1);
        }
      }
    },
    headerStrParser_closure: {
      "^": "Closure:4;headerMap",
      call$1: function(str) {
        var t1, match;
        t1 = J.getInterceptor$asx(str);
        if (t1.get$isNotEmpty(str)) {
          match = t1.split$1(str, ":");
          if (0 >= match.length)
            return H.ioore(match, 0);
          this.headerMap.$indexSet(0, match[0], C.JSString_methods.trim$0(C.JSArray_methods.join$1(C.JSArray_methods.sublist$1(match, 1), ":")));
        }
      }
    },
    sendRequest_closure: {
      "^": "Closure:2;httpComplete,request",
      call$1: function($event) {
        var headers, t1;
        if (J.get$currentTarget$x($event) != null)
          W._convertNativeToDart_EventTarget($event.currentTarget);
        headers = this.request.getAllResponseHeaders();
        t1 = this.httpComplete;
        if (t1.future._state === 0)
          t1.complete$1(0, E.headerStrParser(headers));
      }
    }
  }, 1]];
  setupProgram(dart, 0);
...
;
  ...
  $ = new Isolate();
  init.metadata = [];
  init.types = [{func: 1}, {func: 1, v: true}, {func: 1, args: [,]}, {func: 1, v: true, args: [{func: 1, v: true}]}, {func: 1, args: [P.String]}, {func: 1, ret: P.String, args: [P.$int]}, {func: 1, args: [, P.String]}, {func: 1, args: [{func: 1, v: true}]}, {func: 1, args: [,], opt: [,]}, {func: 1, args: [,,]}, {func: 1, args: [[P.Map, P.String, P.String]]}];
